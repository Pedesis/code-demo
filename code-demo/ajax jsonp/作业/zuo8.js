...
var scoreHigh=function(students){
    var max=0;
    var index=0;
    for(var i=0;i<students.length;i++){
        var s=students[i].score;
        if(s>max){
            max=s;
            index=i;
        }
...
var formated_weekday=function(n){
    switch(n){
        case 1:
        return "星期一";
        case 2:
...
ensure(discount(100, '小学生')==50)
//log(discount(100, '小学生'))
discount(100, '初中生')
discount(100)
    /*
    price 是一个 int
...
var numArray=function(array){
    ...
    var result=[];
    for(var i=0;i<arr.length;i++){
        var max=toLang(array);
        max=max+4;
        log("max",max);
        result[i]=model(arr[i].length,max,arr[i])+"\n";
...
numArray([ 'objective-c', 'python', 'js']);  
    /*
